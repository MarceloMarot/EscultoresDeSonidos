...
Para entender su funcionamiento se analizan los aspectos más importantes de su esquema circuital original y se simulan los subcircuitos más importantes.

## Service Notes (original)

<image src="Imagenes/1-Boss OC2-Service Notes.png" alt="1-Boss OC2-Service Notes.png" >

## Esquema circuital (original)

<image src="Imagenes/2-Boss OC2-Original.png" alt="2-Boss OC2-Original.png" >

El esquema fue simulado con el programa LTSpice. Se dejaron de lado aquellas secciones del circuito que no son exclusivas de este efecto: buffers, circuito de enclavamiento, conmutación analógica , etc. También se reemplazaron los componentes no disponibles por otros parecidos.
...
Para conseguir la segunda octava el diseño original usaba un flip-flop T buffereado , el BA634. Éste también hace la división de frecuencia por dos, y al ser excitado con la primera octava el resultado es una segunda octava lógica, es decir una onda cuadrada con la cuarta parte de la frecuencia de entrada.

<image src="Imagenes/10-BA634.png" alt="10-BA634.png">

Este chip está obsoleto y en muchos diseños derivados se lo reemplaza con un flip-flop JK como el CD4027 o con un segundo flip-flop D como el CD4013.
...
<image src="Imagenes/11-Modulacion.png" alt="11-Modulacion.png" >
...
<image src="Imagenes/filtros salida.png" alt="filtros salida.png" >
...
En la gráfica se muestra la respuesta de los tres circuitos de la primera octava ante un tono puro de entrada de 200Hz. Se acopla la señal de entrada preamplificada al desplazador de nivel (verde), se conmuta en base a la primera octava lógica (amarillo) y  se filtra (rojo):

<image src="Imagenes/Octava 1.png" alt="Octava 1.png" >

El resultado final se parece mucho al de una sinusoide que pasa por una distorsión de cruce (crossover distortion) en el centro lo cual se traduce en distorsión armónica agregada. Además, como la conmutación se produjo muy cerca del cero las discontinuidades producidas son pequeñas, esto se traduce en armónicos de alta frecuencia con poca amplitud y entonces el filtro de salida apenas afecta la forma de onda. Todo esto se verifica con la representación en respecto a la frecuencia:

<image src="Imagenes/octava 1 - fft lin.png" alt="octava 1 - fft lin.png" >
...
El proceso se repite respecto a la segunda octava. Se toma la primera octava, se pasa por el desplazamiento de nivel (rojo) , se hace la conmutación (naranja) y se filtra (bordó):

<image src="Imagenes/octava 2.png" alt="octava 2.png" >

En este caso se nota una mayor deformidad en la forma de onda de salida de la segunda octava respecto al caso de la primera y con discontinuidades muy grandes a debidas a la conmutación. El filtro esta vez hace un suavizado mucho más agresivo. La representacion en el dominio de la frecuencia es el siguiente:

<image src="Imagenes/octava 2 fft lin 2.png" alt="octava 2 fft lin 2.png" >
...
<image src="Imagenes/Todas las salidas.png" alt="Todas las salidas.png" >

Y las componentes de frecuencia de cada señal de salida serían como la siguiente:

<image src="Imagenes/todas las salidas - fft.png" alt="todas las salidas - fft.png" >

## Mezcla de salida:

<image src="Imagenes/mezcla salida.png" alt="mezcla salida" >
...
- Los filtros introducen un desfase que depende de la frecuencia. En el caso del filtro de 3º orden el desfase puede alcanzar hasta 270º (90º x 3) en las frecuencias más altas y la red RC posterior añade un pequeño desfase adicional. Sin embargo este diseño parece no darle importancia a esta cuestión. A continuación se ven las respuestas en fase de los tres filtros:

<image src="Imagenes/filtro entrada - fase.png" alt="filtro entrada -fase" >


## Extra: El esquema de Lehrberg

El esquema que se encuentra más habitualmente online es el trazado por Stellan Lehrberg, el cual sirvió de referencia para muchos proyectos DIY a lo largo del mundo. En este esquema se reemplaza el BA634 con el CD4027 y se quitan los elementos de bypass y buffer del efecto:

<image src="Imagenes/Boss OC2 - esquema Lehrberg.png" alt="Lehrberg" >

Este diagrama es bastante fiel al original, aunque tiene dos detalles importantes:

- El pin 7 del CD4027 (set) aparece conectado a VCC. Sin embargo para que el sistema funcione correctamente este pin debe ser referenciado al negativo, de otro modo este flip-flop se mantiene insensible a la entrada y entonces la segunda octava no se produce.
- Los pines de entrada del operacional G (comparador) están invertidos haciendo que las señales de set y reset se ponen en valor alto simultáneamente forzando a que ambas salidas se pongan a nivel alto. Esto da lugar a una señal de salida del primer flip-flop asimétrica que (sorprendentemente) no afecta al funcionamiento de las etapas posteriores debido a que el flanco ascendente sigue produciéndose en el mismo lugar.

Ambos detalles se marcan a continuación:

<image src="Imagenes/Boss OC2 - esquema Lehrberg MOD.png" alt="Lehrberg MOD" >
